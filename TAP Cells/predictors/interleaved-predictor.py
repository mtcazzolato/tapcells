...
def getInterpolationFunction(previousTimestamps, previousPoints, function, uniSpline_k = 3):
    
    # Perform the correspondent function (default is interp1d, with the 'linear' setting)
    
    if (function == 0): # 0: Univariate Spline
        p = interpolate.UnivariateSpline(previousTimestamps, previousPoints, k = uniSpline_k)
    elif (function == 1): # 1: Cubic Splines
        p = interpolate.CubicSpline(previousTimestamps, previousPoints, extrapolate = 'periodic')
    elif (function == 2): # 2: interp1d - Nearest
        p = interpolate.interp1d(previousTimestamps, previousPoints, kind = 'nearest', fill_value = 'extrapolate')
    elif (function == 3): # 3: Lagrange's
        p = interpolate.lagrange(previousTimestamps, previousPoints)
    elif (function == 4): # 4: RBF with radial basis function 'multiquadric'
        p = interpolate.Rbf(previousTimestamps, previousPoints, function = 'multiquadric')#, smooth = 100)
    elif (function == 5): # 5: RBF with radial basis function 'inverse'
        p = interpolate.Rbf(previousTimestamps, previousPoints, function = 'inverse')
    elif (function == 6): # 6: RBF with radial basis function 'gaussian'
        p = interpolate.Rbf(previousTimestamps, previousPoints, function = 'gaussian')
    elif (function == 7): # 7: RBF with radial basis function 'linear'
        p = interpolate.Rbf(previousTimestamps, previousPoints, function = 'linear')
    elif (function == 8): # 8: RBF with radial basis function 'cubic'
        p = interpolate.Rbf(previousTimestamps, previousPoints, function = 'cubic')
    elif (function == 9): # 9: RBF with radial basis function 'quintic'
        p = interpolate.Rbf(previousTimestamps, previousPoints, function = 'quintic')
    elif (function == 10): # 10: KroghInterpolator
        p = interpolate.KroghInterpolator(previousTimestamps, previousPoints)
    else: # else: interp1d - Linear
        p = interpolate.interp1d(previousTimestamps, previousPoints, kind = 'linear', fill_value = 'extrapolate')
    
    # Return the fitted interpolation function
    return (p)
...
def interpolateAndMatch(it, initialMatch):
    ...
    for i in range(len(pointsBf2)):
        if (pointsBf2['Cell'].iloc[i].getInitialTime() == initialTime):
            ...
            fx = getInterpolationFunction(interpDf['time'].values, interpDf['x'].values, intpFunction, 3)
            fy = getInterpolationFunction(interpDf['time'].values, interpDf['y'].values, intpFunction, 3)
            fz = getInterpolationFunction(interpDf['time'].values, interpDf['z'].values, intpFunction, 3)
            ...
            if (childrenPerCell[seedAtCell['CellID'][initialMatch['CellID'][i]]] == 1):
                ...
                for p in np.arange((it-(w*2)), (it-w)):
                    embryo['Cell'][seedAtCell['CellID'][initialMatch['CellID'][i]]].addSeed(p, fx(p), fy(p), fz(p), 0)
            ...
            # Check if the cell has more than one match
                ...
                for p in np.arange((it-(w*2)), (it-w)):
                    ncell.addSeed(p, fx(p), fy(p), fz(p), 0)
...
if __name__ == "__main__":
    if (len(sys.argv) != 5):
        ...
        print('Usage: <w> <input file with filenames> <path to input files> <output_path>')
    else:
