...
def getRegressionFunction(previousTimestamps, previousPoints, order):
    ...
    pipe = Pipeline(Input)
    pipe.fit(previousTimestamps, previousPoints)
...
def initializeFirstBuffer(filepath, filenames):
    ...
    for it in range(1, w):
        ...
        # output.loc[it] = [it, len(embryo), sum(embryo.active == True)]
        buffer1 += 1
        numberOfNonPredictions += 1
...
def interpolateAndMatch(it, initialMatch):
    ...
    for i in range(len(pointsBf2)):
        if (pointsBf2['Cell'].iloc[i].getInitialTime() == initialTime):
            ...
            # Get a regression model for each dimension separately
            fx = getRegressionFunction(interpDf[['time']], interpDf['x'], order)
            fy = getRegressionFunction(interpDf[['time']], interpDf['y'], order)
            fz = getRegressionFunction(interpDf[['time']], interpDf['z'], order)
            ...
            if (childrenPerCell[seedAtCell['CellID'][initialMatch['CellID'][i]]] == 1):
               # Add interpolated points to the embryo, in the corresponding cells
                for p in np.arange((it-(w*2)), (it-w)):
                    embryo['Cell'][seedAtCell['CellID'][initialMatch['CellID'][i]]].addSeed(p,
                                                                                            fx.predict(p.reshape(1, -1))[0],
                                                                                            fy.predict(p.reshape(1, -1))[0],
                                                                                            fz.predict(p.reshape(1, -1))[0],
                                                                                            0)
...
if __name__ == "__main__":
    if (len(sys.argv) != 6):
        ...
        print('Usage: <w> <input file with filenames> <path to input files> <output_path> <order>')
    else:
