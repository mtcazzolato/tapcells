### 
# BallTree-Predictor: tracking and predicting cells from a developing embryo
...
# # BallTree-Predictor's parameters:
...
# BallTree-Predictor's control variables:
buffer = 0
...
# Get active cells from embryo, given the seed-to-cell correspondence from last BallTree-related iteration
##############################################################################################################
...
# These seeds are going to be used to construct the current BallTree structure
##############################################################################################################
...
def predict(i, buffer, numberOfPredictions, isWindowInitialized):
    ...
    for nc in range(0, nCells):
        ...
        if (activeCells.Cell[nc].getNumberOfSeeds() > 1):
            ...
            # d = computeDistanceCoordinates(npoint.loc[0], finalp.loc[0])
    ...
    while i < sequenceSize:
        ...
        if (buffer < w):
            ...
            if (flag_t_or_p == 1):
                ...
                # Since we predicted points in the last iteration, the balltree structure
                # will be constructed using the last predicted points of active cells
                td = getLastPredictedSeeds(seedAtCell)
                updateBallTree(data = td[['xpos', 'ypos', 'zpos']].copy(), metric_distance = 'euclidean')
...
if __name__ == "__main__":
    if (len(sys.argv) != 9):
        ...
        print('Estimating cells\' trajectories using BallTree-Predictor...')
        main(sys.argv)
        print('Done.')
