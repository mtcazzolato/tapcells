...
def predictNextPoint(previousTimestamps, previousPoints, newPosition, order, weighted):
    ...
    previousTimestamps_pr = poly.fit_transform(previousTimestamps.reshape(-1, 1))
...
class PolynomialPredictor():
    ...
    def getPoint(self, points):
        ...
        newPredictedPoint = predictNextPoint(np.arange(1, index3), # previous timestamps
                                             np.float64(points.iloc[index1 : index2]), # previous positions
                                             np.array(w + 1).reshape(1, -1), # next timestamp
...
def main(argv):
    ...
    isWeighted = (True if argv[8] == "True" else False)
...
if __name__ == "__main__":
    if (len(sys.argv) != 9):
        ...
        print('Usage: <w> <pw> <maxError> <input file with filenames> <path to input files> <output_path> <order> <isWeighted>')
    else:
