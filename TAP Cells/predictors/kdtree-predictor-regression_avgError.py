...
# Threshold parameter (only used for error)
...
def getRegressionFunction(previousTimestamps, previousPoints, order):
    ...
    pipe.fit(previousTimestamps.reshape(-1, 1), previousPoints)
...
class PolynomialPredictor():
    ...
    def getPoint(self, points):
        ...
        function = getRegressionFunction(np.arange(1, index3),
                                         np.float64(points.iloc[index1 : index2]),
                                         order)
        
        # Use function to predict next point
        newPredictedPoint = function.predict(np.array(w + 1).reshape(1, -1))
        
        return int(newPredictedPoint)
    ...
    def checkError(self, x, y, z):
        ...
        lastpoint.loc[0] = [float(self.xyzpoints['xpos'][n]),
                            float(self.xyzpoints['ypos'][n]),
                            float(self.xyzpoints['zpos'][n])]
...
def track(i, filepath, filenames):
    ...
    output.loc[i] = [i, len(embryo), sum(childrenPerCell)]#sum(embryo.active == True)]
...
def predict(i, buffer, numberOfPredictions, isWindowInitialized):
    ...
    for nc in range(0, nCells):
        ...
        if (activeCells.Cell[nc].getNumberOfSeeds() > 1):
            ...
            errOutput = predictors.Predictor[nc].checkError(npoint['xpos'], npoint['ypos'], npoint['zpos'])
            sumError += errOutput
...
if __name__ == "__main__":
    if (len(sys.argv) != 8):
        ...
        print('Usage: <w> <pw> <maxError> <input file with filenames> <path to input files> <output_path> <order>')
    else:
